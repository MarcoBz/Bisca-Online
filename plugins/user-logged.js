            ...
            if(to.path === '/v1/login') return redirect('/v1/user')
                ...
                if (splittedRoute[2] === 'rooms' && splittedRoute[1] === 'v1'){
                    if (splittedRoute.length === 4){
                        app.$fireDb.ref(`rooms/${splittedRoute[3]}`).once('value', (snapshot) => {
                            let checkRoom = snapshot.val()
                            if (checkRoom){
                                app.$fireDb.ref(`rooms/${splittedRoute[3]}/users/${store.state.authUser.uid}`).once('value', (snapshot) => {
                                    let user = snapshot.val()
                                    if (!user)  error({ statusCode: 403, message: "You aren't authorized" })
                                })
                            }
                            else error({ statusCode: 403, message: "This room doesn't exist" })
                        })
                    }
                    ...
                    if (splittedRoute.length === 4){
                        app.$fireDb.ref(`matches/${splittedRoute[3]}`).once('value', (snapshot) => {
                            let checkMatch = snapshot.val()
                            if (checkMatch){
                            ...
                            else error({ statusCode: 403, message: "This match doesn't exist" })
                        })
                    }
                    else return redirect('/v1/user')
                }
