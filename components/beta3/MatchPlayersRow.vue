<template>
  <div class="container-fluid" >
    <div class= "row text-center nopadding row-eq-height backgroundClass" >
      ...
      <div class="nopadding col-2 border" v-if = "player[1].played_card">
          <card class = "card-class" v-bind:cardIndex = "player[1].played_card" />        
      </div>
      <div class="nopadding col-2 border" v-else>               
          <card class = "card-class" v-bind:cardIndex = "53" />  
      </div>
      ...
      <div class="nopadding col-1 border" >
          <button class = "btn btn-primary" @click= "goToTable">See Table</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  props: ['player', 'players', 'match', 'game', 'turn', 'matchName'],
    ...
    methods:{
      goToTable(){
          this.$router.push({
              path: `/beta3/${this.matchName}/${this.player[0]}`
          })
      },
    }
